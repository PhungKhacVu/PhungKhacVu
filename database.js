const fs = require('fs/promises');
const path = require('path');

const dbPath = path.join(__dirname, 'database', 'prompts.json');

// In-memory cache for prompts
// Initialized to null to indicate data hasn't been loaded yet.
let promptsCache = null;

async function readPrompts() {
  // If cache is populated, return a deep copy to prevent external mutation
  if (promptsCache !== null) {
    return JSON.parse(JSON.stringify(promptsCache));
  }

  try {
    const data = await fs.readFile(dbPath, 'utf-8');
    promptsCache = JSON.parse(data);
    return JSON.parse(JSON.stringify(promptsCache));
  } catch (error) {
    console.error("Error reading from database:", error);
    // If file doesn't exist or is empty, return an empty array
    promptsCache = [];
    return [];
  }
}

async function writePrompts(prompts) {
  try {
    // Persist to disk FIRST to ensure durability
    await fs.writeFile(dbPath, JSON.stringify(prompts, null, 2), 'utf-8');

    // Update cache only after successful write
    // Store a deep copy to prevent external references from mutating the cache
    promptsCache = JSON.parse(JSON.stringify(prompts));
  } catch (error) {
    console.error("Error writing to database:", error);
    // If write fails, we do NOT update the cache, keeping it consistent with disk.
  }
}

module.exports = {
  readPrompts,
  writePrompts,
};
