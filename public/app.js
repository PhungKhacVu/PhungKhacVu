document.addEventListener('DOMContentLoaded', () => {
    ...
    const savePrompt = async (promptData, skipRender = false) => {
        if (!promptData.id) delete promptData.id; // Prevent empty string ID overwrite
        ...
        try {
            ...
            // ⚡ Bolt Optimization: Local state update avoids N+1 HTTP request bottlenecks during bulk imports
            // Why: Full data refetches trigger slow O(N) network requests
            // Impact: Bulk import saves are significantly faster
            // Measurement: Compare network requests in DevTools before/after
    ...
    const deletePrompt = async (id) => {
        ...
        try {
            ...
            // ⚡ Bolt Optimization: Local state update avoids N+1 HTTP request bottlenecks
            // Why: Full data refetches trigger slow O(N) network requests
            // Impact: Delete action is significantly faster
            // Measurement: Compare network requests in DevTools before/after
