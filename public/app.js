document.addEventListener('DOMContentLoaded', () => {
    ...
    const savePrompt = async (promptData, skipRender = false) => {
        // Prevent empty string ID from being sent, which breaks the backend
        if (promptData.id === '') {
        ...
        try {
            ...
            // Why: Avoids full network refetch (N+1 bottleneck during bulk operations)
            // Impact: Instant UI updates, saves HTTP request
            // Measurement: Zero network delay for list refresh
    ...
    const deletePrompt = async (id) => {
        ...
        try {
            ...
            // ⚡ Bolt Optimization: Local state update
            // Why: Avoids full network refetch
            // Impact: Instant UI updates, saves HTTP request
            // Measurement: Zero network delay for list refresh
    ...
    importFileInput.addEventListener('change', (e) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                // Why: Passes skipRender=true to batch DOM updates during bulk insert, avoiding N+1 HTTP fetches and re-renders
                // Impact: Drastically speeds up import parsing and rendering
                // Measurement: Total import time drastically reduced, 1 UI render
                const savePromises = importedPrompts.map(p => savePrompt(p, true));
                await Promise.all(savePromises);
                renderPromptList(searchInput.value); // Render final state locally
                renderWelcomeScreen();
            } catch (err) {
