document.addEventListener('DOMContentLoaded', () => {
    ...
    // ⚡ Bolt Optimization: Local state update to prevent N+1 HTTP requests during bulk imports
    // Why: Previously, `savePrompt` called `await fetchPrompts()` on every save, fetching the entire list from the server. During bulk imports, this caused N full refetches.
    // Impact: Eliminates N+1 HTTP requests. Bulk imports are now significantly faster and won't crash the server.
    // Measurement: Network tab should show only POST/PUT requests during import, no GET requests.
    const savePrompt = async (promptData, skipRender = false) => {
        ...
        // Remove empty ID so server generates a new one
        if (!isUpdating) {
    ...
    importFileInput.addEventListener('change', (e) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                renderWelcomeScreen(); // Batch UI render update
            } catch (err) {
