document.addEventListener('DOMContentLoaded', () => {
    ...
    const savePrompt = async (promptData, skipRender = false) => {
        ...
        if (!isUpdating) delete promptData.id; // Prevent empty string ID from overwriting server ID
        ...
        try {
            ...
            // Local state update to avoid full HTTP refetch
            if (isUpdating) {
    ...
    importFileInput.addEventListener('change', (e) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                renderPromptList(searchInput.value); // Re-render once after all local updates
                renderWelcomeScreen();
            } catch (err) {
