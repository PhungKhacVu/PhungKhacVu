document.addEventListener('DOMContentLoaded', () => {
    ...
    const savePrompt = async (promptData) => {
        ...
        try {
            ...
            // ⚡ Bolt: Optimistic UI update to avoid N+1 fetches during bulk operations
            if (isUpdating) {
                allPrompts = allPrompts.map(p => p.id === savedPrompt.id ? savedPrompt : p);
            } else {
                allPrompts.push(savedPrompt);
            }
            renderPromptList(searchInput.value); // Re-render local list
    ...
    const deletePrompt = async (id) => {
        ...
        try {
            ...
            // ⚡ Bolt: Optimistic UI update to avoid unnecessary fetch
