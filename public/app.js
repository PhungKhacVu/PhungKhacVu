document.addEventListener('DOMContentLoaded', () => {
    ...
    const savePrompt = async (promptData, skipRender = false) => {
        if (!promptData.id) delete promptData.id; // Fix backend bug
        ...
        try {
            ...
            // ⚡ Bolt Optimization: Local state update to avoid N+1 refetches
            // Why: Prevents full refetch (`fetchPrompts`) after every save/import
            // Impact: Eliminates a network roundtrip and speeds up bulk operations
            // Measurement: Monitor network requests; should be 1 POST instead of 1 POST + 1 GET
    ...
    const deletePrompt = async (id) => {
        ...
        try {
            ...
            // ⚡ Bolt Optimization: Local state update to avoid refetch
            // Why: Prevents full refetch (`fetchPrompts`) after deletion
            // Impact: Eliminates a network roundtrip
            // Measurement: Monitor network requests; should be 1 DELETE instead of 1 DELETE + 1 GET
    ...
    importFileInput.addEventListener('change', (e) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                // ⚡ Bolt Optimization: Batch local UI updates during bulk import
                // Why: Individual saves cause layout thrashing and N+1 API fetching.
                // Impact: Eliminates one GET request per imported item plus one final GET request
                // Measurement: Network panel should show zero GET requests during import.
