document.addEventListener('DOMContentLoaded', () => {
    ...
    const savePrompt = async (promptData, skipRender = false) => {
        ...
        try {
            ...
            // Why: Prevents N+1 HTTP request bottleneck by updating state locally instead of refetching all prompts
            // Impact: Eliminates O(N) network requests during bulk save operations
            // Measurement: Zero GET /api/prompts requests during savePrompt
    ...
    const deletePrompt = async (id) => {
        ...
        try {
            ...
            // ⚡ Bolt Optimization: Local state update
            // Why: Prevents unnecessary HTTP refetch by updating state locally
            // Impact: Eliminates redundant network request upon deletion
            // Measurement: Zero GET /api/prompts requests during deletePrompt
