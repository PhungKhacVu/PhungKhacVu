document.addEventListener('DOMContentLoaded', () => {
    ...
    const savePrompt = async (promptData, skipRender = false) => {
        ...
        try {
            ...
            // Why: Avoids full fetchPrompts() N+1 API call, critical for bulk imports
            // Impact: Eliminates redundant HTTP requests and speeds up UI feedback
            // Measurement: 1 less API call per save, significantly faster loops
            if (isUpdating) {
    ...
    const deletePrompt = async (id) => {
        ...
        try {
            ...
            // ⚡ Bolt Optimization: Local state update
            // Why: Avoids full fetchPrompts() refetch after deletion
            // Impact: Faster UI update, less server load
            // Measurement: 1 less API call per delete
            allPrompts = allPrompts.filter(p => p.id !== id);

            renderWelcomeScreen();
            renderPromptList(searchInput.value); // Re-apply search filter
        } catch (error) {
            console.error(error);
        }
    };
    ...
    importFileInput.addEventListener('change', (e) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                // ⚡ Bolt Optimization: Batch local updates & skip redundant renders
                // Why: Avoids N+1 HTTP request bottlenecks during bulk import by using our updated savePrompt which mutates local state, and skipRender prevents N re-renders
                // Impact: Scales well with large JSON files, eliminating N redundant refetches and renders
                // Measurement: N fewer fetchPrompts() API calls and N fewer renderPromptList() calls
                const savePromises = importedPrompts.map(p => savePrompt(p, true));
                await Promise.all(savePromises);

                renderWelcomeScreen();
                renderPromptList(searchInput.value); // Render just once with the full updated allPrompts array
            } catch (err) {
