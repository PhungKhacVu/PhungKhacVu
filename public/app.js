document.addEventListener('DOMContentLoaded', () => {
    ...
    const savePrompt = async (promptData, skipRender = false) => {
        ...
        try {
            ...
            // ⚡ Bolt Optimization: Local state update to avoid full list refetch
            // Why: Refetching all prompts on every save/import causes O(N) network bottleneck
            // Impact: Eliminates N+1 HTTP GET requests during bulk operations
            // Measurement: Verify zero GET /api/prompts requests upon prompt save or import
            if (isUpdating) {
    ...
    const deletePrompt = async (id) => {
        ...
        try {
            ...
            // ⚡ Bolt Optimization: Local state update to avoid full list refetch
            // Why: Refetching all prompts on delete causes unnecessary network request
            // Impact: Eliminates HTTP GET request after deletion
            // Measurement: Verify zero GET /api/prompts requests upon prompt delete
    ...
    importFileInput.addEventListener('change', (e) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                // ⚡ Bolt Optimization: Batch saving without re-rendering per item
                // Why: Rendering N times during a bulk import causes severe UI freezing
                // Impact: Freezes avoided, N+1 GET requests avoided via skipRender
